...
class DatesChecker:
    ...
    def _check_is_date_datetime(self):
        if isinstance(self._startDate, datetime):
            self._startDate = datetime.strftime(self._startDate, '%Y-%m-%dT%H:%M:%SZ')[:-10]
    ...
    def _from_only_month_valid_date(self):
        if self._onlyMonth:
            ...
            self._startDate = datetime.strftime(datetime.strptime(self._startDate + "-01", '%Y-%m-%d'), '%Y-%m-%d')
    ...
    def _date_max_check(self, dateFormat):
        ...
        if (self._endDate > present) & (self._startDate > present):
            CoinExceptionAPI({"type": "Dates_out_of_range"}).handle()
    ...
    def _full_date(self):
        try:
            ...
            datetime.strptime(self._startDate, '%Y-%m-%d')
            print("You provided bad date format for that function. We added days (start-date=", self._startDate,
                  ") to your dates!")
        ...
        try:
            ...
            self._end_date_last_day_month()
            datetime.strptime(self._endDate, '%Y-%m-%d')
            print("You provided bad date format for that function. We added days ( end-date=", self._endDate,
                  ") to your dates!")
    ...
    def _dates_placement_check(self):
        ...
        if self._startDate > self._endDate:
            ...
            print("The start date is newer than the end date. The dates have been swapped.")
